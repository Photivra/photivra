...
- `partial-foundation` — some low-level science exists, but the full stage contract is not implemented; spatial camera rotation in the temporal domain plus declared-scale focus breathing, radial distortion, and RGB-channel lateral CA in the lens-field domain are current examples;
